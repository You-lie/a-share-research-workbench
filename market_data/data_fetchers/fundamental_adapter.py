...
def _amount_to_yi(value: Any) -> Optional[float]:
    """Normalize an AkShare financial amount to 100 million yuan (亿元)."""
    if value is None:
        return None

    text = str(value).strip().replace(",", "")
    raw = _safe_float(text.replace("亿元", "").replace("亿", "").replace("万元", "").replace("万", "").replace("元", ""))
    if raw is None:
        return None
    if "亿" in text:
        return raw
...
class AkshareFundamentalAdapter:
    ...
    @staticmethod
    def _extract_abstract_metrics(frame: pd.DataFrame) -> Optional[Dict[str, Any]]:
        """Parse AkShare's transposed financial abstract table by metric and report period."""
        if '指标' not in frame.columns:
            return None

        period_columns = sorted(
            (str(column) for column in frame.columns if re.fullmatch(r'\d{8}', str(column))),
            reverse=True,
        )
        if not period_columns:
            return None

        metric_names = frame['指标'].astype(str)

        def _metric_value(keywords: List[str]) -> Optional[float]:
            matched = frame[metric_names.str.contains('|'.join(keywords), regex=True, na=False)]
            for _, metric_row in matched.iterrows():
        ...
        return {
            ...
            'revenue': _amount_to_yi(_metric_value(['营业总收入', '营业收入'])),
            'net_profit_parent': _amount_to_yi(_metric_value(['归母净利润', '净利润'])),
            'eps': _metric_value(['基本每股收益']),
            'roe': _metric_value(['净资产收益率\\(ROE\\)', '净资产收益率']),
            'gross_margin': _metric_value(['毛利率']),
            'debt_ratio': _metric_value(['资产负债率']),
            'operating_cash_flow': _amount_to_yi(_metric_value(['经营现金流量净额', '经营活动产生的现金流量净额'])),
        }
    ...
    def get_fundamental_bundle(self, stock_code: str) -> Dict[str, Any]:
        ...
        if fin_df is not None:
            abstract_metrics = self._extract_abstract_metrics(fin_df)
            if abstract_metrics is not None:
                revenue_yoy = None
                profit_yoy = None
                roe = abstract_metrics['roe']
                gross_margin = abstract_metrics['gross_margin']
                debt_ratio = abstract_metrics['debt_ratio']
                report_date = abstract_metrics['report_date']
                revenue = abstract_metrics['revenue']
                net_profit_parent = abstract_metrics['net_profit_parent']
                operating_cash_flow = abstract_metrics['operating_cash_flow']
                eps = abstract_metrics['eps']
                capex = None
            else:
                ...
                if row is not None:
                    ...
                    debt_ratio = _safe_float(_pick_by_keywords(row, ["资产负债率"]))
                    report_date = _normalize_report_date(_pick_by_keywords(row, _DIVIDEND_KEYWORD_MAP["report_date"]))
                    revenue = _amount_to_yi(_pick_by_keywords(row, ["营业总收入", "营业收入", "营收"]))
                    net_profit_parent = _amount_to_yi(_pick_by_keywords(row, ["归母净利润", "母公司股东净利润", "净利润"]))
                    operating_cash_flow = _amount_to_yi(
                        _pick_by_keywords(row, ["经营活动产生的现金流量净额", "经营现金流", "经营活动现金流"])
                    )
                    eps = _safe_float(_pick_by_keywords(row, ["基本每股收益", "每股收益", "EPS"]))
                    capex = _amount_to_yi(
                    ...
                    revenue_yoy = profit_yoy = roe = gross_margin = debt_ratio = None
                    report_date = revenue = net_profit_parent = operating_cash_flow = eps = None
                    capex = None

            if any(value is not None for value in (revenue, net_profit_parent, eps, roe, gross_margin, debt_ratio, operating_cash_flow)):
                result["growth"] = {
                ...
                if fin_source == "stock_financial_abstract" and (eps is not None or roe is not None):
        ...
        if cf_df is not None and not cf_df.empty:
            ...
            if cf_row is not None:
                ...
                _capex_raw = _pick_by_keywords(cf_row, ["购建固定资产、无形资产和其他长期资产支付的现金",
                                                        "购建固定资产支付的现金", "资本支出"])
                oper_cf = _amount_to_yi(_oper_cf_raw)
                capex = _amount_to_yi(_capex_raw)
