...
class DataFetcherManager:
            ...
            try:
                ...
                if quote is not None and quote.has_basic_data():
                    ...
                    if primary_quote is None:
                        ...
                        primary_quote = quote
                        self._initialize_quote_field_sources(primary_quote)
    ...
    def _merge_quote_fields(cls, primary, secondary) -> list:
        ...
        *primary* has None. Returns list of field names that were filled.
        """
        cls._initialize_quote_field_sources(primary)
        cls._initialize_quote_field_sources(secondary)
        filled = []
        for f in cls._SUPPLEMENT_FIELDS:
            if getattr(primary, f, None) is None:
                ...
                if val is not None:
                    ...
                    primary.field_sources[f] = secondary.field_sources.get(f, cls._quote_source_id(secondary))
                    filled.append(f)
        return filled

    @staticmethod
    def _quote_source_id(quote) -> str:
        source = getattr(quote, 'source', '')
        return getattr(source, 'value', str(source or 'unknown'))

    @classmethod
    def _initialize_quote_field_sources(cls, quote) -> None:
        if quote is None:
            return
        if not hasattr(quote, 'field_sources') or quote.field_sources is None:
            quote.field_sources = {}
        source = cls._quote_source_id(quote)
        for field_name in (
            'price', 'change_pct', 'change_amount', 'volume', 'amount', 'open_price', 'high', 'low',
            'pre_close', 'volume_ratio', 'turnover_rate', 'pe_ratio', 'pb_ratio', 'total_mv', 'circ_mv', 'amplitude',
        ):
            if getattr(quote, field_name, None) is not None:
                quote.field_sources.setdefault(field_name, source)
